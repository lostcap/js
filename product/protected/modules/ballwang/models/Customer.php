...
//    }
    ...
    public static function returnCustomerEmail($orderId) {
        $custmerArray = array();
        $order = Order::model()->findAllByAttributes(array('order_site_id' => $orderId));
        if ($order) {
            foreach ($order as $key => $value) {
                $custmer = $value->customer;
                $custmerArray[$custmer->customer_email] =$custmer->customer_name ;
            }
        }
        return $custmerArray;
    }
