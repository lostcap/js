...
class OrderController extends BallController {
    ...
    public function accessRules() {
		...
		array('allow', // allow all users to perform 'index' and 'view' actions
                'actions' => array( 'SynOrder', 'SynSiteBack','ReturnAjaxOrder'),
                'users' => array('*'),
            ),
            array('allow', // allow all users to perform 'index' and 'view' actions
            ...
            array('allow', // allow all users to perform 'index' and 'view' actions
                'actions' => array('noShippedAdmin', 'OrderSerach', 'orderSerachByCustomer', 'WaitingPayment', 'OutPutBreakDownWaitingPayment', 'OutPutWaitingPayment','Refund','OrderSerachByCustomerName'),
                'roles' => array('chat'),
            ),
            array('allow', // allow all users to perform 'index' and 'view' actions
                'actions' => array('admin', 'noShippedAdmin', 'orderSerachByCustomer', 'productShippingShow', 'shippingImport', 'noshippedProduct', 'SynSite', 'OrderSerach', 'OutPutOrder','ShowOutputOrder','showEmail','sendEmail','OutPutReportForm','delete'),
    ...
    public function actionView($id) {
        if(isset($_POST['product_sku'],$_POST['product_qty'],$_POST['product_size'],$_POST['product_price'],$_POST['order_site_id'],$_POST['yt1'])){
            $productCollection=ProductCollection::model()->findByAttributes(array('product_sku'=>trim($_POST['product_sku'])));
            if(!$productCollection){
                $productCollection=new ProductCollection();
                if(!$_POST['product_name']){
                    $_POST['product_name']=$_POST['product_sku'];
                }
                $productCollection->product_name=trim($_POST['product_name']);
                $productCollection->product_sku=trim($_POST['product_sku']);
                $productCollection->product_site_id=trim($_POST['order_site_id']);
                $productCollection->save();
            }
            $productAttribute=OrderAttribute::model()->findByAttributes(array('order_attribute_size'=>trim($_POST['product_size'])));
            if(!$productAttribute){
                $productAttribute= new OrderAttribute();
                $productAttribute->order_attribute_size=$_POST['product_size'];
                $productAttribute->save();
            }
            $orderItem=new OrderItem();
            $orderItem->item_qty=trim($_POST['product_qty']);
            $orderItem->item_price=trim($_POST['product_price']);
            $orderItem->item_weight=10;
            $orderItem->item_total=$orderItem->item_qty*$orderItem->item_price;
            $orderItem->item_attribute_id=$productAttribute->order_attribute_id;
            $orderItem->item_product_id=$productCollection->product_id;
            $orderItem->item_product_name=trim($_POST['product_sku']);
            $orderItem->order_id=trim($_POST['order_id']);
            if($orderItem->save()){
                $orderUpdate=Order::model()->findByPk($orderItem->order_id);
                $orderUpdate->order_subtotal +=trim($_POST['product_price']);
                $orderUpdate->order_grandtotal +=trim($_POST['product_price']);
                $orderUpdate->update();
            }
        }
    ...
    public function actionDelete($id) {
        if (Yii::app()->request->isPostRequest) {
            ...
            $orderItem=$this->loadOrderItemModel($id);
            $order=Order::model()->findByPk($orderItem->order_id);
            if($order){
                if(($order->order_subtotal-$orderItem->item_total)>=0){
                    $order->order_subtotal -=$orderItem->item_total;
                    $order->order_grandtotal -=$orderItem->item_total;
                    if($order->save()){
                        $orderItem->delete();
                    }
                }
        ...
        $this->redirect('/ballwang/order/view/id/'.$orderItem->order_id);
    }
                        ...
                        if (!empty($item) && $row > 1 && $item[0]) {
                            ...
                            if ($order) {
                                if($order->order_status!=Order::PaymentAccepted){
                                    ...
                                    $message .=$item[0] . ' 亲,手下留情该单是成功的哦！<br>';
                                }
                            } else {
    ...
    public function actionOrderSerachByCustomerName() {
        ...
        $this->render('serachInvoiceByCustomerName', array(
            'model' => $model,
            'email' => $email,
        ));
    }
    ...
    public function loadOrderItemModel($id) {
...
//                ORDER BY order_payment_at ASC";
                    ...
                    if ($orderSite) {
                        while ($row = mysql_fetch_array($orderSite)) {
                            if ($order->order_ship_id != 0) {
                                ...
                                mysql_query($shipSql);
                                $sql = 'update syo_order set order_status=' . $order->order_status . ',order_valid=1,order_ship_id=(select ship_id from syo_order_ship where ship_code="' . $ship->ship_code . '") where invoice_id="' . $order->invoice_id . '"';
                                $targetSite = mysql_query($sql);
                            } else {
                                if($order->order_status==2||$order->order_status==3||$order->order_status==9){
                                    $sql = 'update syo_order set order_status=' . $order->order_status . ',order_valid=1 where invoice_id="' . $order->invoice_id . '" ;';

                                }else{
                                    $sql = 'update syo_order set order_status=' . $order->order_status . ',order_valid=0 where invoice_id="' . $order->invoice_id . '" ;';
                                }
    ...
    public function actionOutputReportForm() {
        $this->breadcrumbs = array(
            'Orders' => array('index'),
            '报表生成',
        );
        $siteForm = new SiteForm();
        $this->actionMessage = '报表生成';
        $siteForm->getAllOrder(2,false);
        if (isset($_POST['yt0']) && isset($_POST['SiteForm'])) {
            $orders = $siteForm->order;
            Yii::import('application.vendors.*');
            require_once 'PHPExcel/PHPExcel.php';
            require_once 'PHPExcel/PHPExcel/IOFactory.php';
            if ($orders) {
                ...
                $objPHPExcel->setActiveSheetIndex(0)
                    ->setCellValue('A1', '下单时间')->setCellValue('B1', '订单编号')
                    ->setCellValue('C1', '客户姓名')->setCellValue('D1', '客户邮箱')
                    ->setCellValue('E1', 'SKU')->setCellValue('F1', '数量')
                    ->setCellValue('G1', '货运方式')->setCellValue('H1', '追踪号')
                    ->setCellValue('I1', '发货时间')->setCellValue('J1', '状态')
                    ->setCellValue('K1', '订单额(转换前)')->setCellValue('L1', '退款金额(转换前)')
                    ->setCellValue('M1', '币种')->setCellValue('N1', '汇率')
                    ->setCellValue("O1", "订单额(人民币)")->setCellValue("P1",'退款金额(人民币)')
                    ->setCellValue("Q1", "运费(人民币)")->setCellValue("R1",'拿货价(人民币)')
                    ->setCellValue("S1", "利润(人民币)");
                $objPHPExcel->getActiveSheet(0)->setTitle('订单输出');
                $index = 2;
                foreach ($orders as $key => $order) {
                    $orderPrefix = $order->site->domain_prefix;
                    ...
                    $carrier=$order->carrier;
                    $ship=$order->ship;
                    $refundPrice=$order->getPriceByRefund(true);
                    $currency=$order->currency;
                    $getPrice=number_format($refundPrice['order_grandtotal_CNY']-$ship['ship_shipping_cost']-$ship['ship_product_cost']-$refundPrice['order_refund_CNY'],2,'.','');
                    $objPHPExcel->setActiveSheetIndex(0)
                        ->setCellValueExplicit('A' . $index, $order['order_create_at'], PHPExcel_Cell_DataType::TYPE_STRING)
                        ->setCellValueExplicit('B' . $index, '(' . $orderPrefix . ')' . $order['invoice_id'], PHPExcel_Cell_DataType::TYPE_STRING)
                        ->setCellValueExplicit('C' . $index, $address['customer_firstname'] . ' ' . $address['customer_lastname'], PHPExcel_Cell_DataType::TYPE_STRING)
                        ->setCellValueExplicit('D' . $index, $customer['customer_email'], PHPExcel_Cell_DataType::TYPE_STRING)
                        ->setCellValueExplicit('G' . $index, $carrier['carrier_name'], PHPExcel_Cell_DataType::TYPE_STRING)
                        ->setCellValueExplicit('H' . $index, $ship['ship_code'], PHPExcel_Cell_DataType::TYPE_STRING)
                        ->setCellValueExplicit('I' . $index, $ship['ship_create_at'], PHPExcel_Cell_DataType::TYPE_STRING)
                        ->setCellValueExplicit('J' . $index, $ship['ship_status'], PHPExcel_Cell_DataType::TYPE_STRING)
                        ->setCellValueExplicit('K' . $index, $refundPrice['order_grandtotal'], PHPExcel_Cell_DataType::TYPE_STRING)
                        ->setCellValueExplicit('L' . $index, $refundPrice['order_refund'], PHPExcel_Cell_DataType::TYPE_STRING)
                        ->setCellValueExplicit('M' . $index, $currency['currency_code'], PHPExcel_Cell_DataType::TYPE_STRING)
                        ->setCellValueExplicit('N' . $index, $refundPrice['rateToCny'], PHPExcel_Cell_DataType::TYPE_STRING)
                        ->setCellValueExplicit('O' . $index, $refundPrice['order_grandtotal_CNY'], PHPExcel_Cell_DataType::TYPE_STRING)
                        ->setCellValueExplicit('P' . $index, $refundPrice['order_refund_CNY'], PHPExcel_Cell_DataType::TYPE_STRING)
                        ->setCellValueExplicit('Q' . $index, $ship['ship_shipping_cost'], PHPExcel_Cell_DataType::TYPE_STRING)
                        ->setCellValueExplicit('R' . $index, $ship['ship_product_cost'], PHPExcel_Cell_DataType::TYPE_STRING)
                        ->setCellValueExplicit('S' . $index, $getPrice, PHPExcel_Cell_DataType::TYPE_STRING);
                    ...
                    foreach ($productList as $row) {
                        $objPHPExcel->setActiveSheetIndex(0)
                            ->setCellValue('E' . ($index + $i), $row['product_sku'], PHPExcel_Cell_DataType::TYPE_STRING)
                            ->setCellValueExplicit('F' . ($index + $i), $row['item_qty'], PHPExcel_Cell_DataType::TYPE_STRING);
                        $i++;
                    }
                    $index += $i;
                }
                $name = '南京联嵌' . date('Y') . '年' . date('m月d日', strtotime($siteForm->siteStartTime)) . '-' . date('m月d日', strtotime($siteForm->siteEndTime)) . '统计报表.xls';
                $objPHPExcel->setActiveSheetIndex(0);
                header('Content-Type: application/vnd.ms-excel');
                header('Content-Disposition: attachment;filename=' . $name);
                header('Cache-Control: max-age=0');
                ...
                $this->message['error'] = date('Y-m-d', strtotime($siteForm->siteStartTime)) . '到' . date('Y-m-d', strtotime($siteForm->siteEndTime)) . '无满足条件的订单';
            }
        }

        $this->render('outPutReportForm', array('siteForm' => $siteForm));
    }
    ...
    public function actionProductShippingShow() {
        ...
        if (isset($_POST['yt0']) && isset($_POST['OutPutOrderForm'])) {
            ...
            if ($orders) {
                ...
                $objPHPExcel->setActiveSheetIndex(0)
                        ->setCellValue('A1', '下单时间')->setCellValue('B1', '订单编号')
                        ->setCellValue('C1', 'SKU')->setCellValue('D1', '尺码')
                        ->setCellValue('E1', '数量')->setCellValue('F1', '客户姓名')
                        ->setCellValue('G1', '国家')->setCellValue('H1', '省/州')
                        ->setCellValue('I1', '城市')->setCellValue('J1', '地址一')
                        ->setCellValue('K1', '地址二')->setCellValue('L1', '邮编')
                        ->setCellValue('M1', '电话')->setCellValue('N1', '货运方式')
                        ->setCellValue("O1", '币种')->setCellValue("P1", "金额")
                        ->setCellValue("Q1", "交易号");
                $objPHPExcel->getActiveSheet(0)->setTitle('订单输出');
                $index = 2;
                foreach ($orders as $key => $order) {
                 ...
                 // $paypal = $order->paypal;
                    ...
                    $objPHPExcel->setActiveSheetIndex(0)
                            ...
                            ->setCellValueExplicit('M' . $index, $address['address_phonenumber'], PHPExcel_Cell_DataType::TYPE_STRING)
                            ->setCellValueExplicit('N' . $index, $carrier['carrier_name'], PHPExcel_Cell_DataType::TYPE_STRING)
                            ->setCellValueExplicit('O' . $index, $currency['currency_code'], PHPExcel_Cell_DataType::TYPE_STRING)
                            ->setCellValueExplicit('P' . $index, $order['order_grandtotal'], PHPExcel_Cell_DataType::TYPE_STRING)
                            ->setCellValueExplicit('Q' . $index, Order::getOrderStatue($order['order_status']), PHPExcel_Cell_DataType::TYPE_STRING);

                    $sql = "SELECT t1.*,t2.product_name AS pname,t2.product_sku ,t3.order_attribute_size FROM {{order_item}} as t1
                        LEFT JOIN {{product_collection}} as t2 ON t1.item_product_id=t2.product_id
                        LEFT JOIN {{order_attribute}} as t3 ON t1.item_attribute_id=t3.order_attribute_id
                        where t1.order_id={$order['order_id']}";
                    $productList = Yii::app()->db->createCommand($sql)->queryAll();
                    $i = 0;
                    $j = 0;
                    ...
                    foreach ($productList as $row) {
                        $objPHPExcel->setActiveSheetIndex(0)
                                ->setCellValue('C' . ($index + $i), $row['product_sku'], PHPExcel_Cell_DataType::TYPE_STRING)
                                ->setCellValueExplicit('D' . ($index + $i), $row['order_attribute_size'], PHPExcel_Cell_DataType::TYPE_STRING)
                                ->setCellValueExplicit('E' . ($index + $i), $row['item_qty']);
                        if ($array[$row['product_sku']]) {
                            $array[$row['product_sku']][$row['order_attribute_size']] += $row['item_qty'];
                        } else {
                            $array[$row['product_sku']][$row['order_attribute_size']] = $row['item_qty'];
                        }
                        $i++;
                ...
                if ($array) {
                    $objPHPExcel->setActiveSheetIndex(0)
                            ...
                            ->setCellValue('T' . 1, '尺码', PHPExcel_Cell_DataType::TYPE_STRING)
                            ->setCellValue('U' . 1, '数量', PHPExcel_Cell_DataType::TYPE_STRING);
                    foreach ($array as $key1 => $value1) {
                        foreach($value1 as $key2=>$value2){
                            $objPHPExcel->setActiveSheetIndex(0)
                                ->setCellValue('S' . $z, $key1, PHPExcel_Cell_DataType::TYPE_STRING)
                                ->setCellValue('T' . $z, $key2, PHPExcel_Cell_DataType::TYPE_STRING)
                                ->setCellValue('U' . $z, $value2, PHPExcel_Cell_DataType::TYPE_STRING);
                            $z++;
                        }
                    }
                }
    ...
    public function actionShowEmail(){
        ...
        $emailServer=EmailServer::getEmailServerStatus();

        $this->render('showEmail', array(
            'model' => $model,
                ...
                if ($reader) {
                    ...
                    foreach ($data as $row => $item) {
                        if (!empty($item) && $row > 1) {
                            ...
                            if ($order) {
                                if ($order->order_status != 3&& ($order->order_status ==2 || $order->order_status ==9)) {
                                    $ship=OrderShip::model()->findByAttributes(array('ship_order_id'=>$order->order_id));
                                    if(!$ship){
                                        $ship = new OrderShip();
                                        ...
                                        $ship->ship_shipping_cost= Order::getPriceByStringForShipping(trim($item[2]));
                                        $ship->ship_product_cost= Order::getPriceByStringForShipping(trim($item[3]));
                                        if ($ship->save()) {
                                            $order->order_site_option = 1;
                                            $order->order_shipping_syn = 1;
                                            $order->order_email_queue = 3;
                                            $order->order_ship_id = $ship->ship_id;
                                            $order->order_status = 3;
                                            $order->order_export = 0;
                                            $order->update();
                                            $this->message['info'] .=$item[0] . '货运号 '.$item[1].' 导入成功.<br/>';
                                        }else{
                                        ...
                                        if($ship->update()){
                                            ...
                                            $order->order_export = 0;
                                            $order->update();
                                            $this->message['info'] .=$item[0] . '货运号 '.$item[1].'修改成功.<br/>';
                                        }
                                    }
                                } else {
                                    if($order->order_status !=2 && $order->order_status !=9&&$order->order_status !=3){
                                        $this->message['info'].= '<span style="color:red;">系统检测到</span> '.$item[0].' <span style="color:red;">订单未付款请查实并修改状态</span>.<br/>';
                                    }else{
                                        //success,error,info,warning
                                        $email=0;
                                        $shippCost=Order::getPriceByStringForShipping(trim($item[2]));
                                        $productCost=Order::getPriceByStringForShipping(trim($item[3]));
                                        if($order->order_status==3 && ($order->ship->ship_code !=trim($item[1])||$order->ship->ship_shipping_cost!=$shippCost ||$order->ship->ship_product_cost!=$productCost)){

                                            if($order->ship->ship_code !=trim($item[1])){
                                                $order->ship->ship_code=trim($item[1]);
                                                $email=1;
                                                $message.=$item[0] . '订单追踪号：'.$item[1].' 修改成功!<br/>';
                                            }
                                            if($order->ship->ship_shipping_cost!=$shippCost){
                                                $order->ship->ship_shipping_cost=$shippCost;
                                                $message.=$item[0] . '订单运费：'.$item[2].'='.$shippCost.' 修改成功!<br/>';
                                            }
                                            if($order->ship->ship_product_cost!=$productCost){
                                                $order->ship->ship_product_cost=$productCost;
                                                $message.=$item[0] . '订单拿货价：'.$item[3].'='.$productCost.' 修改成功!<br/>';
                                            }
                                            if($order->ship->update()){
                                                $order->order_site_option = 1;
                                                if( $email){
                                                    $order->order_shipping_syn = 1;
                                                    $order->order_email_queue = 3;
                                            ...
                                            $this->message['info'].=$item[0] . '订单追踪号 '.$item[1].'<span style="color:red;"> 已经导入</span>.<br/>';
                                        }
                                    }

                                }
                            } else {
                                $this->message['info'] .=$orerId . '<span style="color:red;">订单号没有找到</span>.<br/>';
                            }
                        }
                    }
                }
